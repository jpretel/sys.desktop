...
public class FrmListaOrdenServicio extends AbstractDocList {
	...
	public Object[][] getData(int idesde, int ihasta, String serie, int numero) {
		lista = ordenDAO.getFiltro(idesde, ihasta, serie, numero);
		data = new Object[lista.size()][3];
		int i = 0;
		for (OrdenServicio oc : lista) {
			...
			data[i] = new Object[] { c.getTime(), oc.getSerie(), cnumero,
					...
					oc.getAlmacen().getDescripcion(), oc.getIdordenservicio() };
			i++;
		}
		return data;
	}
